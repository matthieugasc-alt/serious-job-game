...
**LOT 1 FAIT (couche pure, vert)** : `app/workspace/tools/bibliotheque/{spec,model,api}.ts` calqués mot pour mot sur `bloc-notes/` — 23 ops `tool_op` (index scenario_doc idempotent, archivage mail/fil dédupliqué par source = règle « un choix » du contrat §2, dossiers/tags/pin/favori, surlignage/commentaire/signet, liens doc↔doc bidirectionnels, bureau multi-fenêtres/layout/comparaison), reducer pur/immutable/défensif, sélecteurs dont `searchEntries(state, query, resolveContent)`. Enregistré : TOOL_REGISTRY (icône 🗂️, `applyOp` câblé, panneau placeholder `BibliothequeToolPanel`), schema `tools`, `NON_RESETTABLE_TOOLS` (TS `composerV3` + mjs `validate-scenarios`). 36 tests (model/api/garde-fous/intégration moteur). **Le Tool est câblable et journalisé, mais SANS UI joueur** (juste un aperçu dans le panneau).
**LOT 2 À FAIRE (UI, gros morceau)** : `BibliothequeApp` hébergée par l'app documents (le `DocumentsApp` actuel doit auto-indexer les `DocumentDef` du step via `api.indexScenarioDoc` puis monter la bibliothèque), `ReaderAugmente` (surlignage/commentaires ancrés/signets + bouton « → Bloc-notes » via `bloc-notes/api.ts`), `CompareView`, `FolderTree`, `SearchBar` (branche `searchEntries` avec un `resolveContent` fourni par l'hôte depuis `scenario.documents`), bureau multi-fenêtres, `ArchiveButton` hébergé par Mail (mail ouvert) et Messages (fil/sélection) sur le modèle exact d'`AnnotateButton` — snapshot horodaté construit par l'hôte. Garde-fou à étendre : les apps hôtes ne doivent importer QUE `bibliotheque/ArchiveButton` (comme la règle AnnotateButton dans `workspace.gardefou.test.ts`).
