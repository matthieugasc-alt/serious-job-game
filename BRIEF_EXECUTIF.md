...
**V2 (structure prête)** : moteurs Graph/Timeline/Kanban, presets RACI/MoSCoW/BMC/Ishikawa/roadmap, assistant IA (`/api/v2/decision-assist` — jamais de choix), conversion libre entre TOUTES les vues (V1 = Matrix↔Decision + Registry-risques↔Decision), boutons hôtes Mail/Messages/Bloc-notes, `LinkToDecisionButton` dans Mail/Messages. **Dette** : les presets Table/Registry créés depuis une décision sont des Boards autonomes (items propres), pas encore des vues live des champs de la décision (sauf risques) ; `describeForObservation` neutre mais vue replay dédiée non construite.
